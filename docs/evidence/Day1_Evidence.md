...
### 思考 1：为什么课程没有要求第一天立即开发页面？

第一天不写代码，看似"慢"，实则是**磨刀不误砍柴工**。我认为课程这样设计有以下深层原因：

**① 建立全局视图，避免盲人摸象**

如果一上来就写页面，很容易陷入"只见树木、不见森林"的状态。先通过 Task 1~3 理解项目背景、目录结构、代码骨架，才能知道每一个页面在整个系统中的位置。就像拼图——先看清全图，再动手拼每一块，效率远高于拿起一块就拼。

**② 理解"基础设施"与"业务代码"的边界**

Day 1 让学生区分哪些是课程统一提供的（`.nvmrc`、`eslint.config.ts`、`vite.config.ts` 等），哪些是自己要开发的（`src/` 下 5 个核心目录）。这种"架构组提供脚手架，业务团队在上层开发"的分工模式，恰恰是真实企业的做法。如果第一天就开始改代码，很可能误改基础设施文件，导致环境不一致。

**③ 培养"先规划、后编码"的工程习惯**

Task 5 的项目规划放在编码之前，强迫自己在写第一行代码之前想清楚：有哪些页面、模块怎么划分、按什么顺序做、重点在哪。这在软件工程中就是"概要设计"——它决定了项目的上限。现实中很多项目失败不是因为代码写得差，而是前期想得不清楚。

**④ 模拟真实入职场景**

真实入职第一天，你不会立刻写代码。你会读文档、搭环境、了解架构、熟悉规范。课程 Day 1 的流程——读需求 → 看目录 → 理解代码 → 做规划——还原的正是这个" onboarding"过程。

```
传统教学                         本课程
═══════════════════          ═══════════════════
Day 1: 开始写代码              Day 1: 理解项目全貌
Day 2: 继续写                  Day 2: 规划 + 开始编码
Day N: 写完了                  Day 3-6: 按规划开发
                               Day 7: 验收总结

结果：能写代码                 结果：能写代码 + 理解为什么这样写 + 知道怎么规划
```

---

### 思考 2：软件工程中的"理解项目"为什么比"开始编码"更重要？

这个问题触及了软件工程的核心。我用一个比喻来回答：

> **编码是"施工"，理解项目是"设计图纸"。没有图纸的施工，房子盖得越快，倒塌得越快。**
...
结论：在理解项目阶段发现的问题，修复成本最低。
```

如果在理解阶段就发现"这个功能用户其实不需要"或者"这两个模块耦合太紧"，改一行需求文档的成本几乎为零。但如果已经写了几千行代码再回头改，成本是前者的几十甚至上百倍。

**② 理解项目决定了"做正确的事"，编码只是"正确地做事"**

这是 Peter Drucker 的管理学经典论断在软件工程中的映射：

| 维度 | 含义 | 阶段 |
|------|------|------|
| **做正确的事**（Effectiveness） | 方向对——这个功能用户需要吗？架构合理吗？ | 理解项目 |
| **正确地做事**（Efficiency） | 方法对——代码写得规范吗？性能好吗？ | 编码实现 |

前者是战略问题，后者是战术问题。战术再优秀，也弥补不了战略失误。"最快的编码速度，也救不回一个方向错误的项目。"

**③ 理解项目建立的"心智模型"指导后续所有决策**

当你在写 `ProductCard.vue` 这个组件时，你需要做出无数微决策：

- 这个 Props 叫什么名字？
- 点击事件要 emit 还是用 Router 跳转？
- 数据从父组件传入还是从 Store 读取？

如果你理解了整个项目的架构（路由结构、状态管理方案、组件树），这些决策就有据可依。如果不理解，就只能凭感觉——感觉往往是错的。
...
近 70% 的失败根源在于"理解"阶段，而非"编码"阶段。这就是为什么软件工程如此强调需求分析和架构设计。

---

### 思考 3：AI 在软件开发中应该承担什么角色？

通过今天的 AI 协作实践，我对 AI 的角色有了更清晰的认识。我认为 AI 应该承担以下角色——但有一个绝对不能逾越的边界：
...
| 角色 | 说明 | 今日实例 |
|------|------|---------|
| 🚀 **加速器** | 快速完成机械性、重复性工作 | 分析项目目录结构、生成代码骨架 |
| 🌐 **翻译官** | 在"需求语言"和"技术语言"之间翻译 | 将业务需求转化为页面清单和模块划分 |
| ✅ **检查官** | 审查代码规范、发现潜在问题 | 检查 TypeScript 类型、ESLint 规则符合性 |
| 📝 **模板机** | 生成文档框架、表格结构 | 生成过程证据模板、Markdown 表格 |
| 📚 **知识库** | 回答技术问题、解释概念 | 解释 `createApp().use().mount()` 流程 |

**AI 绝对不能承担的角色：**

| 角色 | 原因 |
|------|------|
| ❌ **决策者** | AI 没有业务上下文，不理解校园场景的真实需求，不能替我做架构决策 |
| ❌ **责任人** | 代码出了问题，AI 不会负责——最终责任人永远是我自己 |
| ❌ **思考替代品** | 如果 AI 替我想清楚了所有问题，我就失去了思考能力——这是本课程要培养的核心能力 |
...
> **一句话总结**：AI 是"副驾驶"（Copilot），不是"自动驾驶"。方向盘始终在自己手里——AI 帮你踩油门、看地图、提醒路线，但往哪开、什么时候转弯，必须自己决定。
...
> **建立了"软件工程全景意识"——知道一个项目不是从写代码开始的，而是从"理解"开始的。**

具体展开：

**收获一：理解了"不动手"的价值**

过去我的习惯是拿到任务就开始写代码，遇到问题再回头查。今天的流程让我意识到：花 80% 时间理解项目、20% 时间写代码，和花 20% 时间理解、80% 时间反复修改——前者的总耗时其实更短、质量更高。

**收获二：项目规划的"地图效应"**

Task 5 做出来的项目规划（8 页面 + 7 模块 + 6 Phase），就像一张地图。有了这张地图，未来 6 天我知道每天该做什么、做到什么程度算完成、什么时候该切换到下一个任务。这种感觉很踏实。

**收获三：AI 协作的"边界感"**

今天最大的意外收获是学会了和 AI 协作的正确姿势。不是"AI 帮我做"，而是"我问 AI → AI 给参考 → 我判断 → 我调整"。这种协作模式不会让自己变懒，反而因为需要判断 AI 的输出质量，思考得更深入。
